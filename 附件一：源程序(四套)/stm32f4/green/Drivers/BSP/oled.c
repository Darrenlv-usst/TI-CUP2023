...
{ 
	...
	OLED_WR_Byte((((x+1)&0xf0)>>4)|0x10,OLED_CMD);           //设置起始列地址的高四位，x+1才能在屏幕上看到完整字符  
	OLED_WR_Byte(((x+1)&0x0f),OLED_CMD);                            //设置起始列地址的低四位，x+1才能在屏幕上看到完整字符  
}    	  
//开启OLED显示
...
{      	
	unsigned char c=0,i=0;	
		c=chr-' ';//在ASIIC码中，空格字符排在第32位，由于其前面的操作符不能显示，故要减去空格字符的ASIIC码值
		...
		if(SIZE ==16)
			...
			OLED_WR_Byte(F8X16[c*16+i+8],OLED_DATA);
				...
				OLED_Set_Pos(x,y);
				for(i=0;i<6;i++)
				OLED_WR_Byte(F6x8[c][i],OLED_DATA);  /*若显示字母‘t’，则会将F6x8数组的第c行的数据一个一个地传到ssd1306，每个数据一个字节
				                                                                        每一个字节的8位代表写入每一页的每一列的8个像素值，从上往下对应从低位到高位，1代                                 
				                                                                        表亮起，0代表熄灭*/
			}
}
